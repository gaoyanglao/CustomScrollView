...
public class CustomVerticleScrollView : MonoBehaviour, IEndDragHandler, IDragHandler, IBeginDragHandler
{
    public Item Item;
    //用户数据
    ...
    private bool sliderUp;//标记当前的滑动方向
    ...
    private void Awake()
    {
        ...
        for (int i = 0; i < showNum+1; i++)
        {
            ...
            if (i == 0)
            {
                size = item.rectTransform.rect.size.y;//得到每一个Item的高
                index0Y = item.transform.localPosition.y;
            }
            else {
                item.transform.localPosition = itemList[i-1].transform.localPosition - dir * size;
            }
            item.gameObject.SetActive(true);
            item.GetComponent<Image>().color = new Color(Random.Range(0,1.0f), Random.Range(0, 1.0f), Random.Range(0, 1.0f));
            itemList.Add(item);
        }
    ...
    {
        offset = eventData.position.y - lastPosition.y;//得到滑动的偏移量
        ...
        if (offset > 0)
        {
            ...
            if (itemList[0].transform.localPosition.y >= index0Y && isMoveItem == false)
            {
            ...
            {
                if (itemList[0].transform.localPosition.y >= index0Y + 200)
                {
                    //第一个完全滑出区域外了
                    startIndex++;
